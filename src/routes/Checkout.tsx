...
export default function Checkout() {
  ...
  const handleConfirm = () => { clearCart(); setStep(2) }
      ...
      <div style={{ padding: `40px ${px}px`, maxWidth: 1100, margin: '0 auto' }}>
        ...
        <AnimatePresence mode="wait">
          {step === 0 && (
            ...
            >
              {isMobile && <OrderSummary items={items} total={total} />}
              <div style={{ padding: isMobile ? 20 : 32, background: 'var(--bg-1)', border: '1px solid var(--border)', borderRadius: 16 }}>
                ...
                <button className="btn-primary" style={{ marginTop: 28, width: '100%', justifyContent: 'center', fontSize: 15, padding: '14px' }} onClick={() => setStep(1)} disabled={!form.email || !form.name || !form.phone}>
          ...
          {step === 1 && (
            ...
            >
              {isMobile && <OrderSummary items={items} total={total} />}
              <div style={{ padding: isMobile ? 20 : 32, background: 'var(--bg-1)', border: '1px solid var(--border)', borderRadius: 16 }}>
                <h2 style={{ fontFamily: '"Space Grotesk",sans-serif', fontSize: 22, fontWeight: 700, marginBottom: 28, letterSpacing: '-0.02em' }}>Método de pago</h2>
                <div style={{ display: 'grid', gap: 10 }}>
                  {[
                    ...
                    <button key={m.id} onClick={() => setForm((f) => ({ ...f, payMethod: m.id }))}
                      style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '16px 18px', background: form.payMethod === m.id ? 'rgba(230,4,18,0.08)' : 'var(--bg-2)', border: `1.5px solid ${form.payMethod === m.id ? '#E60412' : 'var(--border)'}`, borderRadius: 10, cursor: 'pointer', textAlign: 'left', fontFamily: 'inherit', transition: 'all 0.2s', width: '100%' }}
                    >
                      ...
                      {form.payMethod === m.id && <span style={{ marginLeft: 'auto', width: 16, height: 16, borderRadius: '50%', background: '#E60412', flexShrink: 0 }} />}
                    </button>
                  ))}
                </div>
                <div style={{ display: 'flex', gap: 10, marginTop: 28 }}>
                  <button className="btn-ghost" style={{ flex: 1, justifyContent: 'center', fontSize: 14 }} onClick={() => setStep(0)}>Volver</button>
                  <button className="btn-primary" style={{ flex: 2, justifyContent: 'center', fontSize: 15, padding: '14px' }} onClick={handleConfirm}>
          ...
          {step === 2 && (
            <motion.div key="step2" initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} style={{ maxWidth: 560, margin: '0 auto', textAlign: 'center' }}>
              <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: 0.2, type: 'spring', stiffness: 200 }}
                style={{ width: 80, height: 80, borderRadius: '50%', background: 'rgba(74,222,128,0.15)', border: '2px solid rgba(74,222,128,0.4)', display: 'grid', placeItems: 'center', margin: '0 auto 24px', color: '#4ade80' }}
              >
                <CheckCircle size={36} />
              </motion.div>
              <h1 style={{ fontFamily: '"Space Grotesk",sans-serif', fontSize: isMobile ? 28 : 36, fontWeight: 700, letterSpacing: '-0.03em', marginBottom: 12 }}>¡Pedido confirmado!</h1>
              <p style={{ color: 'var(--fg-3)', fontSize: 15, lineHeight: 1.6, marginBottom: 24 }}>Revisá tu mail. Te mandamos las credenciales en menos de 15 minutos.</p>
              <div style={{ display: 'inline-block', padding: '10px 20px', background: 'var(--bg-2)', border: '1px solid var(--border)', borderRadius: 8, fontFamily: 'JetBrains Mono, monospace', fontSize: 18, color: 'var(--fg-0)', letterSpacing: '0.1em', marginBottom: 32 }}>
                #{orderNum}
              </div>
      ...
      <div style={{ display: 'grid', gap: 10, marginBottom: 16 }}>
        {items.map((item) => (
          <div key={`${item.gameId}-${item.accountType}`} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13 }}>
            <span style={{ color: 'var(--fg-1)' }}>{item.title} <span style={{ color: 'var(--fg-3)' }}>×{item.qty}</span></span>
